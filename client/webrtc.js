  ...
  window.setInterval(function() {
    yourConn.getStats(null).then(stats => {
      ...
      stats.forEach(report => {
        ...
        arr.push(statsOutput)//push each timestamp data into ann array

        console.log("arrr :",arr[0]);// accessing first timestamp data
        console.log("last_arrr :",arr.slice(-1)[0]);//acessing last timestamp data
      });
        document.getElementById("stats-box").innerHTML = statsOutput;
    });
  }, 1000);
